...
public class sunMain extends JFrame {
	...
	public void api(String locdate) {
	    try {
	        ...
	        String moonrise = response.substring(response.indexOf("<sunrise>") + 9, response.indexOf("</sunrise>"));
	        String moonset = response.substring(response.indexOf("<sunset>") + 8, response.indexOf("</sunset>"));
	        System.out.print(moonrise);
	        System.out.print(moonset);
	        dtm.setRowCount(0);
	        String[] info = new String[2];
	        info[0] = moonrise.substring(0, 2)+"시 "+moonrise.substring(2, 4)+"분";
	        info[1] = moonset.substring(0, 2)+"시 "+ moonset.substring(2, 4)+"분";
	        dtm.addRow(info);
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
